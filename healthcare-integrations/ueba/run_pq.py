...
def _expand_let_base(text: str) -> str:
    """
    Rewrite the concise `let base = ( ... ); | union (base | columns ...),
    (base | columns ...), ... | <tail>` form into the inlined-union form
    that this tenant's parser accepts:
    ...
    If the file does not use `let base = ( ... )` the text is returned as-is.
    """
    import re
    m = re.search(
        r'^\s*let\s+(\w+)\s*=\s*\(\s*\n?(.*?)\n?\s*\)\s*;',
    ...
    # Strip leading `|` lines that come from the documented form
    body_lines = []
    for ln in body.splitlines():
        body_lines.append(ln)
    body = "\n".join(body_lines).strip()
    # Replace `<name>` references in the remainder with the inlined body.
    # The remainder is everything after the `let ... );`.
    rest = text[m.end():]
    # Pattern: ( <name> | columns ... )  →  ( <body>  | columns ... )
    # We do a simple regex sub on word-boundary `name` only when followed
...
def split_into_branches(pq_text: str) -> list:
    ...
    Files written in the concise `let base = ( ... ); | union (base | ...)`
    form are first expanded to the inlined-union form by `_expand_let_base`.
    """
    import re
    text = strip_comments(pq_text)
    text = _expand_let_base(text)
